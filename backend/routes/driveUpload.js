...
router.post('/', authMiddleware, async (req, res) => {
    try {
        ...
        DriveDownloader.download(url, destPath, (downloaded, total) => {
            ...
            // Use custom name from request, or derive from ZIP filename
            let chatName = req.body.customChatName?.trim();

            if (!chatName) {
                const zipBasename = path.basename(destPath, path.extname(destPath));
                chatName = zipBasename !== 'raw' ? zipBasename.replace(/^WhatsApp Chat with\s+/i, '').trim() : `Drive Import ${new Date().toISOString().split('T')[0]}`;
            }
