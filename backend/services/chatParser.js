...
class ChatParser {
    constructor() {
        ...
        this.timestampPatterns = [
            // [M/D/YY, H:MM:SS PM] Sender: Message (Matches the user's provided format exactly)
            // Handles invisible LTR marks, narrow no-break spaces (\u202F), and standard spaces
            /^\[(\d{1,2}\/\d{1,2}\/\d{2,4}),?\s+(\d{1,2}:\d{2}(?::\d{2})?)[\s\u00A0\u202F\u2009]*([AaPp][Mm])\]\s*([^:]+?):\s*(.*)$/,

            // M/D/YY, H:MM pm/am - Sender: Message
    ...
    parseMessageLine(line) {
        for (const pattern of this.timestampPatterns) {
            ...
            if (match) {
                ...
                // Handle the new 6-group format (date, time, am/pm, sender, body)
                if (match.length === 6) {
                    [, date, time, ampm, sender, body] = match;
                    time = time + ' ' + ampm;  // Combine time and am/pm
                } else if (match.length === 5) {
                    // Handle traditional 5-group format
